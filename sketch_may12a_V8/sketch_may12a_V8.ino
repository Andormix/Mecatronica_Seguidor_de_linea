...
#define VEL_MIN 180 // Mínima velocitat
#define VEL_MIN_E 197 // Miníma velocitat motor M2 (Correcció de desviament)
...
void loop() {
  ...
  if (velEsq > VEL_MAX)
    ...
    velDre = velDre;         // Reduim velocitat a un 60%
    velEsq = 0;
  }
  if (velDre > VEL_MAX)
    velDre = VEL_MAX;
...
void intermitents(int err)
{
  if (err <= 1000 && err >= -1000) // Linia mes o menys al centre
  {
    digitalWrite(L_ESQUERRA , HIGH);   
    digitalWrite(L_DRETA , HIGH);  
  }
  else if( err > 1000) // Linea a la dreta
  {
    digitalWrite(L_ESQUERRA , HIGH);   
    digitalWrite(L_DRETA , LOW); 
  }  
  else // Linia a la esquerra
  {
    digitalWrite(L_ESQUERRA , LOW);   
    digitalWrite(L_DRETA , HIGH); 
  }
}